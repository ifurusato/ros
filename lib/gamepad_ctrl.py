...
class GamepadController():
    ...
    '''
    def __init__(self, config, pid_motor_controller, video, blob, matrix11x7_available, level, close_callback):
        super().__init__()
        if config is None:
        ...
        _controllers = pid_motor_controller.get_pid_controllers()
...
#           self.set_led_show_battery(True)
    ...
    def add(self, message):
        ...
        if not self._enabled:
            self._log.warning('queue disabled: message ignored #{}: priority {}: {}'.format(message.number, message.priority, message.description))
            return
...
#           pids = self._pid_motor_ctrl.set_max_velocity(_mapped_velocity)
            ...
            self._log.info(Fore.BLUE + Style.BRIGHT  + 'event: {};\tvalue: {:5.2f}cm'.format(event.description, message.value))
...
#               time.sleep(0.5) # debounce
            ...
            if message.value == 0:
                ...
                if self._cruise_behaviour is None:
                    ...
                    self.add_consumer(self._cruise_behaviour)
                    self._cruise_behaviour.cruise()
                    self._log.info(Fore.MAGENTA + 'enabled cruise.')
                else:
                    self.remove_consumer(self._cruise_behaviour)
